function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
    ...
    // Sort plainTextPositions in descending order to avoid offset issues during modification
    plainTextPositions.sort((a, b) => b.start - a.start);
  
    // Apply the highlights by adding <mark> tags to the corresponding positions in htmlContent
    for (const position of plainTextPositions) {
      const { start, end } = position;
      const plainTextToHighlight = plainText.substring(start, end);
  
      // Find the corresponding positions in the htmlContent
      const startHTML = highlightedContent.indexOf(plainTextToHighlight);
      const endHTML = startHTML + plainTextToHighlight.length;
  
      // Add <mark> tags to the found positions in htmlContent
      const highlightedText = `<mark>${highlightedContent.substring(startHTML, endHTML)}</mark>`;
      highlightedContent =
        highlightedContent.substring(0, startHTML) +
        highlightedText +
        highlightedContent.substring(endHTML);
    }
